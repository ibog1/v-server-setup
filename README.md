...
A complete, structured setup of a Linux V‑Server for secure SSH access and an NGINX website, documented in Markdown with a clickable table of contents.<br>  This document follows the checklist: each step is in its own section, testing is explicit, and no secrets are committed.
# Contents
*	Overview
*	Prerequisites
...
*	Appendix
# Overview
This guide configures the server so SSH uses keys only, NGINX serves a custom index and 404 page, and Git/GitHub are set up over SSH for version control of documentation and safe config where appropriate.<br>
Prerequisites
...
git@github.com:YOUR_ACCOUNT/v-server-setup.git
git add.
...
Negative (should fail): ssh -o PubkeyAuthentication=no user@SERVER
